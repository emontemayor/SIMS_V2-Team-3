...
00105$:
	setb	_PORTC_3
	C$main.c$173$2$247 ==.
;	main.c:173: if (st->error == AXRADIO_ERR_RETRANSMISSION)
	mov	dpl,r6
	mov	dph,r7
	inc	dptr
	movx	a,@dptr
...
00120$:
	clr	_PORTC_3
	C$main.c$186$2$247 ==.
;	main.c:186: if (st->error == AXRADIO_ERR_NOERROR) {
	mov	dpl,r6
	mov	dph,r7
	inc	dptr
	movx	a,@dptr
...
00138$:
	C$main.c$199$2$247 ==.
;	main.c:199: } else if (st->error == AXRADIO_ERR_TIMEOUT) {
	cjne	r5,#0x03,00139$
	C$main.c$200$5$259 ==.
;	main.c:200: led2_on();
	setb	_PORTA_2
00139$:
	C$main.c$210$2$247 ==.
;	main.c:210: if (st->error == AXRADIO_ERR_BUSY)
	cjne	r5,#0x02,00149$
	C$main.c$211$4$261 ==.
;	main.c:211: led3_on();
	setb	_PORTA_5
...
;	main.c:269: ANALOGA = 0x18; // PA[3,4] LPXOSC, other PA are used as digital pins
	...
	movx	@dptr,a
	C$main.c$272$1$271 ==.
;	main.c:272: PORTA = 0xC0 | (PINA & 0x25); 	// pull-up for PA[6,7] which are not bonded, no pull up for PA[3,4] (LPXOSC). Output 0 in PA[0,1,2,5] to prevent current consumption in all DIP switch states
	mov	a,#0x25
	anl	a,_PINA
	orl	a,#0xc0
	mov	_PORTA,a
	C$main.c$274$1$271 ==.
;	main.c:274: PORTB = 0xFE; //PB[0,1]  (LCD RS, LCD RST) are overwritten by lcd2_portinit(), enable pull-ups for PB[2..7]  (PB[2,3] for buttons, PB[4..7] unused)
	mov	_PORTB,#0xfe
	C$main.c$275$1$271 ==.
;	main.c:275: PORTC = 0xF3 | (PINC & 0x08); 	// set PC0 = 1 (LCD SEL), PC1 = 1 (LCD SCK), PC2 = 0 (LCD MOSI), PC3 =0 (LED), enable pull-ups for PC[4..7] which are not bonded Mind: PORTC[0:1] is set to 0x3 by lcd2_portinit()
	mov	a,#0x08
	anl	a,_PINC
	orl	a,#0xf3
	mov	_PORTC,a
	C$main.c$277$1$271 ==.
;	main.c:277: PORTR = 0xCB; // overwritten by ax5043_reset, ax5043_comminit()
	mov	_PORTR,#0xcb
	C$main.c$280$1$271 ==.
;	main.c:280: DIRA = 0x27; // output 0 on PA[0,1,2,5] to prevent current consumption in all DIP switch states. Other PA are inputs, PA[3,4] (LPXOSC) must have disabled digital output drivers
	mov	_DIRA,#0x27
	C$main.c$281$1$271 ==.
;	main.c:281: DIRB = 0x03; // PB[0,1] are outputs (LCD RS, LCD RST), PB[2..7] are inputs (PB[2,3] for buttons,  PB[4..7]  unused)
	mov	_DIRB,#0x03
	C$main.c$282$1$271 ==.
;	main.c:282: DIRC = 0x0F; // PC[0..3] are outputs (LCD SEL, LCD,SCK, LCD MOSI, LED), PC[4..7] are inputs (not bonded).
	mov	_DIRC,#0x0f
	C$main.c$283$1$271 ==.
;	main.c:283: DIRR = 0x15; // overwritten by ax5043_reset, ax5043_comminit()
	mov	_DIRR,#0x15
	C$main.c$296$1$271 ==.
