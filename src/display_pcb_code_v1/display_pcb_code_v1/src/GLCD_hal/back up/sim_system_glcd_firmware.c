...
void lcd_spi_init(void)
{
    ...
    lcd_init_master_config.pinmux_pad1 = LCD_CS;
  ...
  //  lcd_init_master_config.mode_specific.master.baudrate = LCD_SPI_MAX_CLOCK;
		...
		const SERCOM_SPI_CTRLA_Type ctrla = {
			...
			.bit.DOPO = 1, // MOSI on PAD[2], SCK on PAD[3], SS_ on PAD[1]
			.bit.MODE = 3  // Master
		};
		
		SERCOM4->SPI.CTRLA.reg = ctrla.reg;
		const SERCOM_SPI_CTRLB_Type ctrlb = {
			.bit.RXEN = 1,   // RX enabled
			.bit.MSSEN = 1,  // HW SS disabled
			.bit.CHSIZE = 0 // 8-bit character size
		};
		SERCOM4->SPI.CTRLB.reg = ctrlb.reg;
		
		// Mux for SERCOM4 PA08,PA10,PA11
		const PORT_WRCONFIG_Type wrconfig = {
			...
			.bit.PMUXEN = 1,
			.bit.HWSEL = 0,
			.bit.PINMASK = (uint16_t)(PORT_PB12 | PORT_PB13 | PORT_PB14 | PORT_PB15)
		};
