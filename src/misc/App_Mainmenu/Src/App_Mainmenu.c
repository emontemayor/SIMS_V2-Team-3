...
uint16_t displayImageWidth, displayImageHeight;
	...
	char *apps[] = {"1_H.jpg","2_H.jpg","3_H.jpg","4_H.jpg","5_H.jpg","6_H.jpg","7_H.jpg","8_H.jpg","9_H.jpg","10_H.jpg","11_H.jpg","12_H.jpg"};
#else
...
void Load_Thumbnails()
{
	...
	uint16_t		
					fsize=0,
					blklen = 10*1024, bytesRead, bufferChunkSz,
					wsize, imageWidth, imageHeight, imageStride, imageSize;
...
//	unsigned int  numBytesRead;
	...
	do
	{
		...
		if(ifile==NULL)
		{
			...
			Gpu_CoCmd_MemSet(phost,(imn*imageSize)+50*1024l,random(10*1024L),imageSize);
		...
		else
		{
			...
			Gpu_Hal_WrCmd32(phost,(imn*imageSize)+50*1024L);
		...
		while(fsize > 0)
		{
			...
			fsize -= wsize;
			Gpu_Hal_WrCmdBuf(phost,temp,wsize);				/* copy data continuously into command memory */
#else
			...
			fsize -= wsize;
#endif
			Gpu_Hal_WrCmdBuf(phost,temp,bytesRead);				/* copy data continuously into command memory */
#endif
		}
		imn++;
	}while(imn<12);
	...
	App_WrDl_Buffer(phost,BITMAP_SOURCE(50*1024));	
	App_WrDl_Buffer(phost,BITMAP_LAYOUT(RGB565,imageStride, imageHeight));	
#if defined(FT81X_ENABLE)
	App_WrDl_Buffer(phost,BITMAP_LAYOUT_H(imageStride>>10, imageHeight>>9));	
#endif
	App_WrDl_Buffer(phost,BITMAP_SIZE(NEAREST, BORDER, BORDER, imageWidth, imageHeight));	
#if defined(FT81X_ENABLE)
	App_WrDl_Buffer(phost,BITMAP_SIZE_H(imageWidth>>9, imageHeight>>9));	
#endif
	...
	do
	{
		if (f_open(&FilSrc, apps[imn],  FA_READ | FA_OPEN_EXISTING) != FR_OK){
				fsize = 0;
				Gpu_CoCmd_Dlstart(phost);        // start
				Gpu_CoCmd_MemSet(phost,(imn*imageSize)+50*1024l,random(10*1024L),200*50);
			...
			//	f_lseek(&FilSrc,0,SEEK_END);
				...
				Gpu_Hal_WrCmd32(phost,CMD_LOADIMAGE);
			//	Gpu_Hal_WrCmd32(phost,(imn*69120)+50*1024l);
				Gpu_Hal_WrCmd32(phost,(imn*imageSize)+50*1024l);
				Gpu_Hal_WrCmd32(phost,0);
			}
			while(fsize > 0)
			{
				...
				if(fsize < bufferChunkSz && numBytesRead > 0){
					...
					Gpu_Hal_WrCmdBuf(phost,temp,numBytesRead);
	...
	App_WrDl_Buffer(phost,BITMAP_HANDLE(0));
	App_WrDl_Buffer(phost,BITMAP_SOURCE(50*1024));
	//App_WrDl_Buffer(phost,BITMAP_LAYOUT(RGB565,240*2,144));
	//App_WrDl_Buffer(phost,BITMAP_SIZE(NEAREST, BORDER, BORDER, 240, 144));
	App_WrDl_Buffer(phost,BITMAP_LAYOUT(RGB565,imageStride,imageHeight));
#if defined(FT81X_ENABLE)
	App_WrDl_Buffer(phost,BITMAP_LAYOUT_H(imageStride>>10,imageHeight>>9));
#endif
	...
	else
	{
       ...
       else
       {
              ...
              while(FileLen > 0)
              {
                     ...
                     Gpu_Hal_WrMem(phost,Ft800_addr,pbuff,blocklen);
                     Ft800_addr += blocklen;
              }
...
void Backgroundanimation_1()
{
	...
	if(!init)
	{
		...
		App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST, BORDER, BORDER, 100, 120));
		App_WrCoCmd_Buffer(phost,BITMAP_HANDLE(6));	
		App_WrCoCmd_Buffer(phost,BITMAP_SOURCE(232*1024L));	
		App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT(L4,10,40));	
		App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST, BORDER, BORDER, 40, 80));
		App_WrCoCmd_Buffer(phost,BITMAP_HANDLE(7));	
		App_WrCoCmd_Buffer(phost,BITMAP_SOURCE(242*1024L));	
		App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT(L4,10,24));	
		App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST, BORDER, BORDER, 40, 48));
		App_WrCoCmd_Buffer(phost,BITMAP_HANDLE(8));	
		App_WrCoCmd_Buffer(phost,BITMAP_SOURCE(255*1024L));	
		App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT(L8,512,1));	
		App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST, REPEAT, REPEAT, 512, 480));
#endif
	}
	...
	for(j=0;j<3;j++)
	{
		...
		for(i = 0;i<10;i++)
		{
			...
			if(bitmap_handle[j*10+i]==5)
				App_WrCoCmd_Buffer(phost,CELL(_cell));else 
			{
				switch(j)
				{
					case 0:
...
void Backgroundanimation_2()
{
...
#endif
	{
		...
		//y1offset = 272;
		y1offset = DispHeight;
		if(x1offset > DispWidth)
		{
			y1offset = DispHeight - (x1offset - DispWidth);
			x1offset = DispWidth;
		}
		if(xoffset < 0)
		{
		...
		App_WrCoCmd_Buffer(phost,VERTEX2F(x1offset*16,y1offset*16));
	}
#if defined(DISPLAY_RESOLUTION_WVGA)
	for(i=1;i<8;i++)
#else
...
#endif
	{
		...
		if(x1offset < 0)
		{
			y1offset = DispHeight + x1offset;
			x1offset = 0;
		}
		if(xoffset > DispWidth)
		{
			yoffset = (xoffset - DispWidth);
			xoffset = DispWidth;
		}
		//App_WrCoCmd_Buffer(phost,VERTEX2II(xoffset,yoffset,0,0));
		//App_WrCoCmd_Buffer(phost,VERTEX2II(x1offset,y1offset,0,0));
...
void Backgroundanimation_3()
{
	...
	//App_WrCoCmd_Buffer(phost,SCISSOR_SIZE(800,480));
	...
	//Gpu_CoCmd_Gradient(phost,0,DispHeight/2,0xc4cdd2,0,DispWidth,0x4f7588);
	...
	for(i=0;i<20;i++)
	{
		...
		{
#if defined(DISPLAY_RESOLUTION_WVGA)
			App_WrCoCmd_Buffer(phost,POINT_SIZE(16*(60 + (3*i/2))));	
#else
	...
	for(i = 0; i<20 ; i++)
	{
		...
		if(iteration_cts[i]<rate_cts[i])iteration_cts[i]+=inc; else{ iteration_cts[i] = 0;}
	}
	return;
}
...
#if defined(DISPLAY_RESOLUTION_WVGA)
	App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT(L8, DispWidth, 1));
#if defined(FT81X_ENABLE)
	App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT_H(DispWidth>>10, 0));
#endif
	App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST, BORDER, REPEAT, DispWidth, DispHeight));
#if defined(FT81X_ENABLE)
...
void Backgroundanimation_5()
{
	...
	if(!firebubbles.init)
	{
		...
		for(i=0;i<20;i++)
		{
			...
			firebubbles.yoffset_array[i] = random(85) + DispHeight-100;
#else
			firebubbles.xoffset_array[i] = random(DispWidth);
			firebubbles.yoffset_array[i] = random(50) + DispHeight-75;
#endif
		...
		App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST, BORDER, BORDER, DispWidth, 80));
#if defined(FT81X_ENABLE)
		App_WrCoCmd_Buffer(phost,BITMAP_SIZE_H(DispWidth>>9, 0));
#endif
		...
		App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST, REPEAT, BORDER, DispWidth, 80));
#if defined(FT81X_ENABLE)
		App_WrCoCmd_Buffer(phost,BITMAP_SIZE_H(DispWidth>>9, 0));
#endif
	}	
...
#if defined(DISPLAY_RESOLUTION_WVGA)
	App_WrCoCmd_Buffer(phost,BITMAP_TRANSFORM_A(64));	
...
void Backgroundanimation_6()
{
...
#if defined(DISPLAY_RESOLUTION_WVGA)
	App_WrCoCmd_Buffer(phost,LINE_WIDTH(25*16));
#else
	...
	App_WrCoCmd_Buffer(phost,BEGIN(FTPOINTS));
#if defined(DISPLAY_RESOLUTION_WVGA)
	App_WrCoCmd_Buffer(phost,POINT_SIZE(120*16));
#else
...
/* Save the graphics context before enter into background animatioon*/
	...
	/* Restore the graphics context */
        ...
        App_WrCoCmd_Buffer(phost,BITMAP_HANDLE(2));		  
       	App_WrCoCmd_Buffer(phost,BITMAP_SOURCE(50*1024));	
	    App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT(RGB565,displayImageWidth*2, displayImageHeight));	
        #if defined(FT81X_ENABLE)
	    App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT_H(displayImageWidth*2>>10, displayImageHeight>>9));	
        #endif
	    App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST, BORDER, BORDER, displayImageWidth*2, displayImageHeight*2));	
        #if defined(FT81X_ENABLE)
	    App_WrCoCmd_Buffer(phost,BITMAP_SIZE_H(displayImageWidth*2>>9, displayImageHeight>>9));	
        #endif
		
		App_WrCoCmd_Buffer(phost,CELL(iconno-1));
		App_WrCoCmd_Buffer(phost,BITMAP_TRANSFORM_A(128));
		App_WrCoCmd_Buffer(phost,BITMAP_TRANSFORM_E(128));
		App_WrCoCmd_Buffer(phost,VERTEX2F(((DispWidth-(displayImageWidth*2))/2)*16,((DispHeight-(displayImageHeight*2))/2)*16));	
		App_WrCoCmd_Buffer(phost,BITMAP_TRANSFORM_A(256));
		App_WrCoCmd_Buffer(phost,BITMAP_TRANSFORM_E(256));
		App_WrCoCmd_Buffer(phost,RESTORE_CONTEXT());
        ...
        App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST, BORDER, BORDER, 64, 64  ));
        Gpu_CoCmd_LoadIdentity(phost);
	    Gpu_CoCmd_Scale(phost, 65536*2,65536*2);
	    Gpu_CoCmd_SetMatrix(phost );
...
// for points
	...
	while(1)
	{
		...
		App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST,BORDER,BORDER,displayImageWidth,displayImageHeight)); 
#if defined(FT81X_ENABLE)
		App_WrCoCmd_Buffer(phost,BITMAP_SIZE_H(displayImageWidth>>9,displayImageHeight>>9)); 
#endif
		...
		for(i=0;i<=noof_frame;i++)
		{
			...
			if((Ox+DispWidth-20) > 1023)
				...
				App_WrCoCmd_Buffer(phost,VERTEX2F((Ox+DispWidth-20)*16,(int16_t)(DispHeight*0.75)*16));
#else
			App_WrCoCmd_Buffer(phost,VERTEX2F((Ox+DispWidth-20)*16,(int16_t)(DispHeight*0.75)*16));
...
void menu_loopback()
{
...
#if defined(DISPLAY_RESOLUTION_WVGA)
	scroller_init((DispWidth*no_frames-displayImageWidth*3)*16);
#else
	...
	while(1)
	{
...
#endif
		{
			...
			{      
				App_WrCoCmd_Buffer(phost,VERTEX2F((Ox-1)*16,(Oy-1)*16));	
				App_WrCoCmd_Buffer(phost,VERTEX2F((displayImageWidth+Ox+1)*16,(displayImageHeight+Oy+1)*16));					// i pixels wide than image width +1  
			}
		}
...
void menu_win8()
{
	...
	uint16_t image_height = displayImageHeight/*50*/,image_width = displayImageWidth/*100*/,rectangle_width,rectangle_height; 
	...
	char *menudetails[]={"Music",  "Gauges ",  "Gradient",  "Photo",  "Metaball",  "Notepad",  "Signature",  "Sketch","Swiss","Waves","Player","Clocks"};
				...
				else
				{
					...
					{	
						...
						case 1:							
								...
								#if defined(DISPLAY_RESOLUTION_WVGA)
									App_WrCoCmd_Buffer(phost,VERTEX2F((Ox + (rectangle_width-displayImageWidth)/2),(25+Oy)));	
								#else
